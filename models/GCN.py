...
class GCNBlock(nn.Module):
    def __init__(self, input_dim, output_dim, bn=False, add_self=True, normalize_embedding=True,
            dropout=0., relu=0, bias=True):
    ...
    def forward(self, x, adj):
        ...
        # Apply Batch Normalization directly
        if self.bn:
            # Flatten the tensor to [B*L, C]
            batch_size, seq_length, num_features = y.shape
            y_flat = y.view(-1, num_features)  # Shape: [B*L, C]

            # Apply Batch Normalization
            y_flat = self.bn_layer(y_flat)

            # Reshape back to [B, L, C]
            y = y_flat.view(batch_size, seq_length, num_features)
