...
class Attention(nn.Module):
    ...
    def forward(self, x, out_k=None, out_v=None):
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, num_classes=2, embed_dim=64, depth=3,
                 num_heads=8, mlp_ratio=2., qkv_bias=False, mlp_head=False, drop_rate=0.
                 , attn_drop_rate=0.):
        super().__init__()
        ...
        if mlp_head:
            ...
            self.head = Mlp(embed_dim, int(embed_dim * 0.5), num_classes)
        else:
...
class Middle_VisionTransformer(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
